...
public class MailSendService{
	 ...
	 * 发送邮件
	 */
	public void sendMail(String[] to,String title,String mailBody,String  level){
		...
		try{
			...
			//创建邮件
            MimeMessage message = createMimeMessage(session,title,mailBody,to,level);
            //连接
	...
	private MimeMessage createMimeMessage(Session session,String title, String mailBody,String[] tos) throws Exception {
        ...
        message.setFrom(new InternetAddress(mailConfig.getFrom(), "API可用性", "UTF-8"));
        for(String to : tos){
	...
	private MimeMessage createMimeMessage(Session session,String title, String mailBody,String[] tos , String level) throws Exception {
		MimeMessage message = new MimeMessage(session);
		message.setFrom(new InternetAddress(mailConfig.getFrom(), "API可用性", "UTF-8"));
		for(String to : tos){
			message.addRecipient(MimeMessage.RecipientType.TO, new InternetAddress(to, "", "UTF-8"));
		}
		message.setSubject("【报警级别】:"+level+" "+title+mailConfig.getSuffix(), "UTF-8");
		message.setContent(mailBody, "text/html;charset=UTF-8");
		message.setSentDate(new Date());
		message.saveChanges();
		return message;
