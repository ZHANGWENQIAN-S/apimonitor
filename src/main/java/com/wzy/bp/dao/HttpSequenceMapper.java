...
public interface HttpSequenceMapper {
	...
	@Insert("INSERT INTO http_sequence(guid,`group`,`type`,`name`,remark,jobName,frequency,responseTime)  VALUES("
	...
	@Update("update http_sequence t set t.`group`=#{httpSequence.group},t.`type`=#{httpSequence.type},"
			+ "t.`name`=#{httpSequence.name},t.remark=#{httpSequence.remark},t.frequency=#{httpSequence.frequency},"
			+ "t.responseTime=#{httpSequence.responseTime} WHERE t.`guid`= #{httpSequence.guid}")
	...
	@Select("select t.guid,t.`group`,t.type,t.`name`,t.frequency,t.enabled,t.responseTime from http_sequence t where t.archived = 0")
