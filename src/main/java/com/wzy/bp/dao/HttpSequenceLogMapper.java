...
public interface HttpSequenceLogMapper {
	...
	@Select("select * from http_sequence_log t where t.pguid = #{pguid} order by t.createTime desc limit #{num}")
	List<HttpSequenceLog> selectLastFewLogByPguid(@Param("pguid")String pguid,@Param("num")Integer num);
